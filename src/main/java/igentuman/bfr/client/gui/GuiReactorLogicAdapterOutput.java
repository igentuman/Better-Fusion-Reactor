...
public class GuiReactorLogicAdapterOutput extends GuiMekanismTile<TileEntityReactorLogicAdapter> {

    private List<GuiReactorLogicButton> typeButtons = new ArrayList<>();
    private int buttonID = 0;

    public GuiReactorLogicAdapterOutput(InventoryPlayer inventory, final TileEntityReactorLogicAdapter tile) {
    ...
    public void initGui() {
        ...
        for (TileEntityReactorLogicAdapter.ReactorLogic type : TileEntityReactorLogicAdapter.ReactorLogic.values()) {
            if(Objects.equals(type.getDirection(), "in")) continue;
            int typeShift = 22 * i;
            i++;
    ...
    protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
        ...
        renderScaledText(LangUtils.localize("gui.reactor_logic.output_tab"), 36, 20, 0x404040, 117);
